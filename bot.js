...
function getTagIds(channel, genreIds) {
  ...
  const defaultTag = availableTags.find(
    (tag) => tag.name === config.defaultMovieTag
  );
  if (defaultTag) {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "🔗 Links",
          value: `[TMDB](${movie.tmdbUrl})${
            movie.trailerUrl ? ` • [Trailer](${movie.trailerUrl})` : ""
          }`,
        }
      )
      .setTimestamp()
  ...
  try {
    ...
    if (interaction.user.id !== authorId) {
      return await interaction.reply({
        content: "❌ Only the post author can delete this post.",
        ephemeral: true,
      });
    }
