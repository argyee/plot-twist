...
async function handleWatchlistButton(interaction) {
    try {
        ...
        logger.debug("Watchlist button pressed", {
            movieId,
            userId,
        ...
        if (bullyMessage) {
            ...
            logger.debug("Watchlist action blocked by bullying system", {
                movieId,
                userId,
                user: interaction.user.tag,
            });
        ...
        if (alreadyInWatchlist) {
            ...
            logger.debug("Removed movie from watchlist", {
                movieId,
                userId,
                user: interaction.user.tag,
                watchlistCount,
            ...
            logger.debug("Added movie to watchlist", {
                movieId,
                movieTitle,
                movieYear,
                userId,
            ...
            if (
                ...
                logger.info("Watch party threshold reached", {
                    movieId,
                    movieTitle,
                    watchlistCount,
                    threshold: config.watchParty.threshold,
                    threadId: thread.id,
                });
            }
        }
    } catch (error) {
        logger.error("Error handling watchlist button", {
            error: error.message,
            stack: error.stack,
