...
logger.info("Database path determined", {dbPath, nodeEnv: process.env.NODE_ENV});
...
function addToWatchlist(userId, movieId, movieTitle, movieYear, status) {
    const stmt = db.prepare(`
        INSERT
        OR IGNORE INTO watchlist (user_id, movie_id, movie_title, movie_year, status)
    VALUES (?, ?, ?, ?, ?)
    `);
    ...
    return wasAdded; // Returns true if new entry was added
}
...
function removeFromWatchlist(userId, movieId, status) {
    ...
    const wasRemoved = result.changes > 0;

    logger.debug("Remove from watchlist", {
        userId,
        movieId,
        status,
        wasRemoved,
...
function getUserWatchlist(userId, status) {
    ...
    const results = stmt.all(userId, status);

    logger.debug("Get user watchlist", {
        userId,
        status,
...
function linkOverseerAccount(
    ...
    const stmt = db.prepare(`
        INSERT
        OR REPLACE INTO overseerr_links
    ...
    const wasLinked = result.changes > 0;

    logger.debug("Link Overseerr account", {
        discordUserId,
        overseerUserId,
        overseerUsername,
        plexUsername,
...
function unlinkOverseerAccount(discordUserId) {
    try {
        ...
        logger.debug("Unlink Overseerr account", {discordUserId});

        return true;
    } catch (error) {
        logger.error("Error unlinking Overseerr account", {
            error: error.message,
            stack: error.stack,
            discordUserId,
        });
