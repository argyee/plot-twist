...
const data = new SlashCommandBuilder()
  .setName("myrequests")
  .setDescription("View your Overseerr movie requests");
...
async function execute(interaction) {
  ...
  const embed = new EmbedBuilder()
    .setTitle("📥 Your Movie Requests")
    .setColor(0x01d277)
    .setTimestamp()
    .setFooter({
      text: `Linked as ${link.overseerr_username || link.plex_username}`,
    });
  ...
  if (approved.length > 0) {
    const approvedList = approved
      ...
      .join("\n");
    embed.addFields({
      name: `🔵 Approved/Processing (${approved.length})`,
      value: approvedList,
      inline: false,
    });
  }
  ...
  if (requests.length > totalShown) {
    embed.setDescription(
      `Showing ${totalShown} of ${requests.length} requests`
    );
  }
